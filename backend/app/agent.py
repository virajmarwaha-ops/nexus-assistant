...
SYSTEM_PROMPT = (
    ...
    "reply to the operator in plain text summarizing what happened. Don't "
    "repeat a tool call that already took an action (opening an app, "
    "sending a message) — but DO call an information-lookup tool (like the "
    "time) fresh whenever it's asked again, since its value can change; "
    "when you do, always state the actual value plainly (e.g. 'It's 8:45 "
    "PM') rather than commenting on the call itself or on whether the "
    "result changed since last time.\n\n"
